...
const ALLOWED_PROGRAMMES = new Set(['blues', 'pastel', 'grd']);
...
async function handleContact(body) {
  ...
  const email = (raw.email || '').toString().replace(/[\r\n\t]/g, '').trim().slice(0, 254);
  ...
  const programmeRaw = (raw.programme || '').toString().trim();
  const programmeValues = programmeRaw ? programmeRaw.split(',').map(p => p.trim()).filter(Boolean) : [];
  if (programmeValues.some(p => !ALLOWED_PROGRAMMES.has(p))) {
    return { statusCode: 400, body: { ok: false, message: 'Valeur programme invalide.' } };
  }
  const programme = programmeValues.join(', ');
  const payload = {
    prenom,
    nom,
    email,
